...
def llm_evaluator(query, response, ref_answer, model, template, args):
    ...
    output = evaluator.invoke({"query": query, "response": response, "ref_answer": ref_answer})
...
def count_iterations(data, eval_result):
    ...
    print("average # of iterations for successful completion: {}".format(int(sum(num_iter) / len(num_iter))))
