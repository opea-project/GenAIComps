...
def get_trade_info():
    class TradeInfoInput(BaseModel):
        ...
        info_to_seek: str = Field(description="can only be one of the following: open_price, close_price, low_price, high_price, average_price, trading_volume, num_transaction.")
        from_date: str = Field(description="the start date for the aggregates, must be in the YYYY-MM-DD format.")
        span: str = Field(description="the time span for the aggregates, can be 'day', 'week', 'month', 'quarter' or 'year.")
...
def tools_descriptions():
    ...
    tools += [get_duckduckgo_search(), get_interest_calculator(), get_profit_calculator(), get_ticker_lookup(), get_trade_info()]
    return tools