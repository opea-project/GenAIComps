...
if __name__ == "__main__":
    ...
    # HOME="/mnt/localdisk4"
    HOME="/home/demo/"
    image_paths = [Image.open(os.path.join("../rfcs/avatar1.jpg")),
                   Image.open(os.path.join("../rfcs/avatar5.png")),
                   Image.open(os.path.join("../rfcs/pallavi.png"))]
    ...
    with demo:
        ...
        # UI Components
        # Title & Introduction
        gr.Markdown("<h1 style='font-size: 36px;'>Using OPEA to implement a RAG-Powered Human-Like AI Avatar Audio Chatbot</h1>")
        ...
        with gr.Row():
            with gr.Column(scale=8):
                ...
                <p style='font-size: 20px;'>Welcome to our AI Avatar Audio Chatbot! This application leverages OPEA (Open Platform for Enterprise AI) to provide you with a human-like conversational experience. It employs the AudioQnA megaservice to generate an expert answer based on your query, and then animates the avatar figure with output audio. Feel free to interact with the AI avatar by choosing your own avatar and talking into the mic. </p>
                            """)
                gr.Image("../rfcs/flowchart.png", label="Megaservice Flowchart")
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    gr.Markdown(f"""
                    <img src='data:image/png;base64,{gaudi_base64}' alt='Intel®Gaudi' style='width: 100px; height: auto;'>""", label="Intel®Gaudi")
                    gr.Markdown(f"""
                    <img src='data:image/png;base64,{xeon_base64}' alt='Intel®Xeon' style='width: 100px; height: auto;'>""", label="Intel®Xeon")       
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                audio_time_text = gr.Textbox(label=f"Audio processing time", value="0.0 seconds")
            with gr.Column(scale=2):
                video_output = gr.Video(label="Your AI Avatar video: ", format="mp4", width=1280, height=720)
                video_time_text = gr.Textbox(label=f"Video processing time", value="0.0 seconds")
        ...
        # State transitions
        for i, image_path in enumerate(image_paths):
            image_click_buttons[i].click(update_selected_image_state,
        ...
        submit_button.click(
            initial_process,
            inputs=[audio_input],
            outputs=[audio_output_interm, 
                     interm_state,
                     audio_time_text], # need to change interm_state
        )
        interm_state.change(
            final_process,
