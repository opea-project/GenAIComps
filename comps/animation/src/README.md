...
```bash
docker build -t opea/wav2lip:latest -f comps/animation/src/integrations/dependency/Dockerfile .
```

- Gaudi2 HPU

```bash
docker build -t opea/wav2lip-gaudi:latest -f comps/animation/src/integrations/dependency/Dockerfile.intel_hpu .
```
...
```bash
cd GenAIComps
python3 comps/animation/src/integrations/dependency/check_animation_server.py
```
