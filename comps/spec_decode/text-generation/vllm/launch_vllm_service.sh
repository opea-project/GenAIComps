...
spec_model_name=${8:-$default_spec_model}
...
if [ "$hw_mode" = "gpu" ]; then
    docker run -d --rm --gpus all --name="vllm-service" -p $port_number:8008 -e OMPI_MCA_btl_vader_single_copy_mechanism=none --cap-add=sys_nice --ipc=host -e HTTPS_PROXY=$https_proxy -e HTTP_PROXY=$https_proxy -e HF_TOKEN=${HUGGINGFACEHUB_API_TOKEN} opea/vllm:gpu /bin/bash -c "export VLLM_CPU_KVCACHE_SPACE=40 && python3 -m vllm.entrypoints.openai.api_server --enforce-eager --model $model_name   --speculative_model $spec_model_name --num_speculative_tokens 5 --gpu_memory_utilization 0.8 --use-v2-block-manager --seed 42 --tensor-parallel-size $parallel_number --host 0.0.0.0 --port 8008 --block-size $block_size --max-num-seqs  $max_num_seqs --max-seq_len-to-capture $max_seq_len_to_capture "
else
    docker run -d --rm --name="vllm-service" -p $port_number:80 --network=host -v $volume:/data -e HTTPS_PROXY=$https_proxy -e HTTP_PROXY=$https_proxy -e HF_TOKEN=${HUGGINGFACEHUB_API_TOKEN} -e VLLM_CPU_KVCACHE_SPACE=40 opea/vllm:cpu --model $model_name --host 0.0.0.0 --port 80 --speculative_model $spec_model_name --num_speculative_tokens 5 --gpu_memory_utilization 0.8 --use-v2-block-manager --seed 42 --tensor-parallel-size $parallel_number
fi
