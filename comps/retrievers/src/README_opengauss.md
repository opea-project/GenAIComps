...
You need to setup your own VectorDB service (openGauss in this example), and ingest your knowledge documents into the vector database.

As for openGauss, you could start a docker container using the following commands.
...
```bash
docker run -d --name="retriever-opengauss" -p 7000:7000 --ipc=host -e http_proxy=$http_proxy -e https_proxy=$https_proxy -e GS_CONNECTION_STRING=$GS_CONNECTION_STRING  -e INDEX_NAME=$INDEX_NAME -e TEI_EMBEDDING_ENDPOINT=$TEI_EMBEDDING_ENDPOINT -e RETRIEVER_COMPONENT_NAME=$RETRIEVER_COMPONENT_NAME opea/retriever:latest
```
...
export service_name="retriever-opengauss"
