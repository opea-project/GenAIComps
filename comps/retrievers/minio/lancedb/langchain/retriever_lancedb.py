...
logger = CustomLogger("retriever_lancedb")
...
@register_microservice(
    name="opea_service@retriever_lancedb",
    service_type=ServiceType.RETRIEVER,
    endpoint="/v1/retrieval",
    host="0.0.0.0",
    port=7000,
)
@register_statistics(names=["opea_service@retriever_lancedb"])
async def retrieve(input: EmbedDoc) -> SearchedDoc:
    ...
    statistics_dict["opea_service@retriever_lancedb"].append_latency(time.time() - start, None)
    if logflag:
        logger.info(result)
    return result
...
if __name__ == "__main__":
    ...
    if MOSEC_EMBEDDING_ENDPOINT:
        ...
        if logflag:
            logger.info(f"[ retriever_lancedb ] MOSEC_EMBEDDING_ENDPOINT:{MOSEC_EMBEDDING_ENDPOINT}")
        embeddings = MosecEmbeddings(model=MOSEC_EMBEDDING_MODEL)
    elif TEI_EMBEDDING_ENDPOINT:
        # create embeddings using TEI endpoint service
        if logflag:
            logger.info(f"[ retriever_lancedb ] TEI_EMBEDDING_ENDPOINT:{TEI_EMBEDDING_ENDPOINT}")
