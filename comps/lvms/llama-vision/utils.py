...
import torch
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, MllamaForConditionalGeneration, AutoProcessor
...
def setup_distributed_model_tp(args, model_dtype, model_kwargs, logger, cache_dir):
    print(">>>>>>>>>>>>>>>>>>>>>>>>. setup_distributed_model_tp")
    ...
    model = MllamaForConditionalGeneration._from_config(config, torch_dtype=model_dtype, **model_kwargs)
...
def setup_distributed_model(args, model_dtype, model_kwargs, logger):
    ...
    if load_to_meta:
        ...
        with deepspeed.OnDevice(dtype=model_dtype, device="meta"):
            ...
            model = MllamaForConditionalGeneration._from_config(config, torch_dtype=model_dtype)
        ...
        path_to_write_json = merged_model_dir if args.peft_model is not None else args.model_name_or_path
        write_checkpoints_json(
        ...
        with deepspeed.OnDevice(dtype=model_dtype, device="cpu"):
            if args.peft_model is not None:
                ...
                #     args.model_name_or_path, torch_dtype=model_dtype, **model_kwargs
                # )
                model = MllamaForConditionalGeneration.from_pretrained(args.model_name_or_path, torch_dtype=model_dtype, **model_kwargs)
    model.eval()
    ...
    print(f">>>>>>>>> before deepspeed.init_inferen>> model = {model}")
    print(f">>>>>>>>>> deepspeed.init_inference, config is {ds_inference_kwargs}")
    model = deepspeed.init_inference(model, **ds_inference_kwargs)
    model = model.module
    if model.config.model_type in ["llama", "falcon", "qwen2", "starcoder2", "mllama"]:
...
def setup_generation_config_new(args, model, assistant_model, tokenizer):
    bad_words_ids = None
    ...
    print(f">>>>>>>>>>>>>>>>>>>>>>>>> in setup_generation_config_new, type(generation_config) = {type(generation_config)}")
...
def initialize_model(args, logger):
    ...
    generation_config = setup_generation_config_new(args, model, assistant_model, tokenizer)
