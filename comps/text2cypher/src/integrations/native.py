...
from cypher_utils import cypher_cleanup, cypher_insert, prepare_chat_template
...
from gaudiutils import initialize_model, setup_parser
from load_llm import load_llm
from cypher_utils import CypherQueryCorrector2, construct_schema
...
class OpeaText2Cypher(OpeaComponent):
    ...
    async def invoke(self, input: Input):
        ...
        cypher_prompt = PromptTemplate(
            input_variables=["schema"], template=prepare_chat_template(prompt)
        )
        ...
        cypher_generation_chain = LLMChain(llm=self.chat_model, **use_cypher_llm_kwargs,)
