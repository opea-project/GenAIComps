...
```bash
cd ../../../../../../../
docker build -t opea/embedding-multimodal-bridgetower-hpu:latest --build-arg EMBEDDER_PORT=$EMBEDDER_PORT --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/3rd_parties/bridgetower/src/Dockerfile.intel_hpu .
cd comps/3rd_parties/bridgetower/deployment/docker_compose/
...
```bash
cd ../../../../../../../
docker build -t opea/embedding-multimodal-bridgetower:latest --build-arg EMBEDDER_PORT=$EMBEDDER_PORT --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/3rd_parties/bridgetower/src/Dockerfile .
cd comps/3rd_parties/bridgetower/deployment/docker_compose/
