...
adaclip_parser = argparse.ArgumentParser(prog="adaclip", description="PyTorch implementation of Transformer Video Retrieval")
...
adaclip_parser.add_argument("--output_dir", type=str, default="output", help="dir to store model checkpoints & training meta.")
...
# ========================= Model Configs ==========================
adaclip_parser.add_argument("--policy_backbone", default="mobilenet_v3_large", type=str, choices=["raw", "clip", "frozen_clip", "resnet50", "mobilenet_v2", "mobilenet_v3_large"], help="type of visual backbone for policy")
adaclip_parser.add_argument("--clip_backbone", default="ViT-B/32", type=str, choices=["ViT-B/32", "ViT-B/16"], help="type of visual backbone for CLIP")
adaclip_parser.add_argument("--rnn", default="transformer", type=str, choices=["lstm", "bilstm", "transformer"], help="type of RNN backbone")
...
adaclip_parser.add_argument("--mlp_type", type=str, default="mlp", choices=["mlp", "fc"], help="type of linear model to use before gumbel softmax")
adaclip_parser.add_argument("--rescale_size", default=56, type=int, help="Rescale size for using pixel differences (no CNN backbone)")
...
adaclip_parser.add_argument("--sim_header", default="meanP", choices=["meanP", "seqTransf", "transformer"], help="similarity header to aggregate frame features")
adaclip_parser.add_argument("--word_agg", default=None, choices=["mlp", "transformer"], help="method to learn weights for word aggregation")
adaclip_parser.add_argument("--word_agg_temp", type=float, default=1, help="temperature parameter used in word aggregation softmax")
adaclip_parser.add_argument("--frame_agg", default=None, choices=["mlp", "transformer", "qscore"], help="method to learn weights for frame aggregation")
adaclip_parser.add_argument("--frame_agg_temp", type=float, default=1, help="temperature parameter used in frame aggregation softmax")
...
adaclip_parser.add_argument('--concat_captions', default="concat", choices=["concat", "indep-one", "indep-all"], help='concatenate video captions')
adaclip_parser.add_argument("--max_img_size", type=int, default=224, help="max image longer side size, shorter side will be padded with zeros")
adaclip_parser.add_argument("--num_frm", type=int, default=2, help="#frames to use per video")
adaclip_parser.add_argument("--num_frm_subset", type=int, default=0, help="ablation study: number of frames to sample from num_frm frames")
adaclip_parser.add_argument("--sampling", type=str, default="uniform", choices=["uniform", "random"], help="how to sample frames")
...
adaclip_parser.add_argument("--warmup_proportion", default=0.1, type=float, help="proportion of training to perform linear learning rate warmup for. E.g., 0.1 = 10%% of training")
...
adaclip_parser.add_argument("--freeze_cnn", action="store_true", help="freeze CNN by setting the requires_grad=False for CNN parameters.")
adaclip_parser.add_argument("--freeze_clip", action="store_true", help="freeze CLIP by setting the requires_grad=False for CLIP parameters.")
...
adaclip_parser.add_argument("--data_subset", default=0, type=int, help="debug mode. Use only this number of samples for training and testing")
adaclip_parser.add_argument("--no_output", action="store_true", help="do not save model and logs")
adaclip_parser.add_argument('--gradient_accumulation_steps', type=int, default=1, 
...
adaclip_parser.add_argument('--optuna',action='store_true', default=False, help='whether use optuna to tune parameters')
adaclip_parser.add_argument('--n_trials', type=int, default=100, help='Tune times of optuna')
adaclip_parser.add_argument("--do_training_af_optuna", action="store_true", help="whether use optuna tuned parameters to train after optuna tuning")
...
def run_sft_adaclip(
    ...
    if args.optuna:
        ...
        study = optuna.create_study(sampler=sampler, pruner=optuna.pruners.MedianPruner(
            n_warmup_steps=args.optuna_cfg.n_warmup_steps), direction='maximize', study_name="adaclip_bitfit_coef_lr00205_weight_decay00105_n_trials30", storage=storage_name, load_if_exists=True)
