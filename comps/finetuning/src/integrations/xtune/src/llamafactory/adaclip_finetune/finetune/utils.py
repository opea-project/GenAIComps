...
def transform_importance_for_probs(vals, cfg):
    if cfg == 'none':
        ...
        if method == 'power':
            ...
            transformed = [max(0, numpy.tanh((logval - logmiu) / (logstd * hparams[0]) + hparams[1])) for logval in logvals]
        elif method == 'sigmoid':
            logvals = numpy.log(vals)
            logmiu = numpy.mean(logvals[logvals > -numpy.inf])
            logstd = numpy.std(logvals[logvals > -numpy.inf])
            transformed = [(lambda x: 1/(1 + numpy.exp(-x)))(numpy.tanh((logval - logmiu) / (logstd * hparams[0]) + hparams[1])) for logval in logvals]
        elif method == 'uniform':
            ...
            raise NotImplementedError(f'Probability normalization method: {method} is not implmented.')
        return transformed
...
def clip_param_to_be_kept(param_name, cfg):
    ...
    if cfg.peft.method == 'bitfit':
        return param_name.endswith('.bias') or \
            any(kw in param_name for kw in cfg.peft.config.keep_module_keywords)
...
def write_init_params_for_optimization(model, output_dir):
    with open(os.path.join(output_dir, 'init_clip_params_for_optimization.txt'), 'w') as f:
        ...
        for n, p in model.clip.named_parameters():
            if p.requires_grad:
                f.write('{: <64s}\t{}\n'.format(
                    n, ','.join([f'{_}' for _ in p.size()]) if p.size() else '1'
                ))
...
def get_num_params(optimizer, model, clip=False):
    ...
    for group in param_groups:
        for p in group['params']:
            ...
            if p.requires_grad:
                num_tunable += numel
    num_params = sum([p.numel() for p in model.clip.parameters()]) if clip else sum([p.numel() for p in model.parameters()])
...
_importance_metric = {
    'angle': CosineBasedAngle(), 
    'l2norm': lambda a, b: (a - b).norm()
}


def l2norm_retouch(dists):
    return [
        (n, dist / numpy.sqrt(dim), dim)
        for n, dist, dim in dists
    ]
...
def importance_nvd_from_weights_update(params, init_params, param_names, metric='l2norm', update_count=None):
    ...
    for n in param_names:
        ...
        if dim > 1:
            ...
            dist = 0. if metric == 'angle' else (p - p0).abs().item()
...
def group_by_kmeans(nvd, num_groups=2):
    kmeans = KMeans(n_clusters=num_groups, random_state=0, n_init="auto").fit(
        [(numpy.log(d), v) for _, v, d in nvd]
    )
    grouped_vals = [
        [_[1] for i, _ in enumerate(nvd) if g == kmeans.labels_[i]]
...
def group_by_dim(nvd):
    ...
    grouped_vals = [
        [_[1] for i, _ in enumerate(nvd) if g == labels[i]]
        for g in range(len(dims_sorted))
    ]
...
def groupwise_normalization(nvd, num_groups=2, stats_drop_mul_th=0.001):
    ...
    grouped_vals = [[v for v in vals if v > m * stats_drop_mul_th] for m, vals in zip(init_miu, grouped_vals)] # remove zeros
