...
from dassl.utils import load_pretrained_weights, load_checkpoint, l2_norm, sinkhorn_assignment, get_accuracies, get_one_to_one_features
...
def cross_validation_beta_procrustes(cfg, visual_features, class_prototypes, labels, num_of_tries = 25, num_samples = 3, five_crop = False):
    ...
    if five_crop:
        ...
        mask = torch.cat([mask.to("cpu"), torch.zeros(visual_features.shape[0] - mask.shape[0])], dim=0).bool().to(device)
    ...
    for beta in tqdm(betas):
        ...
        for _ in range(num_samples):
            ...
            acc = get_accuracies(
                cfg, new_train_array, new_test_array, transform=transfm)
...
def mapping_refinement(cfg, loss_func, init_transfm, train_arrays, test_arrays, train_visual_feats, train_text_feats, class_prototypes, labels, batch_size, momentum = 0.9, return_ema_transform = False, verbose = True):
    device = cfg.TRAINER.COOP.XPU_ID if cfg.TRAINER.COOP.XPU else cfg.TRAINER.COOP.CUDA_ID
    ...
    if batch_size is None:
        batch_size = num_instances if cfg.TRAINER.LFA.FIVE_CROP else int(
            num_instances * 0.75)
    ...
    for num_iter in tbar:
        ...
        if cfg.TRAINER.LFA.GAUSSIAN_NOISE > 0.0 and np.random.uniform() > 0.5:
            train_visual_batch += (
                torch.randn_like(train_visual_batch) * cfg.TRAINER.LFA.GAUSSIAN_NOISE
            )

        if cfg.TRAINER.LFA.DROP_OUT > 0.0 and np.random.uniform() > 0.5:
            train_visual_batch = torch.nn.functional.dropout(
                train_visual_batch, p=cfg.TRAINER.LFA.DROP_OUT
            )
...
def iterative_unsupervised_refinement(cfg, loss_func, train_arrays, test_arrays, th = 0.0):
    ...
    for n_iter in tbar:
        ...
        tbar.set_description(
            f"Iter: {n_iter} | Used examples: {mask.sum()}/{N}")
    ...
    def test(self, split=None):
        ...
        if not self.cfg.TRAINER.LFA.unsupervised:
            ...
            refined_transfm, ema_transform = mapping_refinement(
                self.cfg,
                loss_func=partial(adaptive_reranking_loss, knn=self.cfg.TRAINER.LFA.KNN, scale=self.cfg.TRAINER.LFA.arerank_scale),
                init_transfm=transfm,
                train_arrays=train_arrays,
                test_arrays=test_arrays,
