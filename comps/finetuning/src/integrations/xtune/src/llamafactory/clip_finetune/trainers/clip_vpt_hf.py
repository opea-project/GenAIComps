...
class PromptEncoder(nn.Module):
    def __init__(self, cfg, classnames, transformer_model):
        ...
        self.promptt_embeddings = nn.Parameter(torch.zeros(
        ...
        if self.cfg.TRAINER.COOP.PMT_DEEP:
            ...
            self.deep_promptt_embeddings = nn.Parameter(torch.zeros(
                    self.total_d_layer, self.prompt_length, prompt_dim))
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if idx == 0 or self.cfg.TRAINER.COOP.PMT_DEEP != True:
                    ...
                    if idx <= self.deep_promptt_embeddings.shape[0]:
                        deep_prompt_emb = self.promptt_dropout(self.promptt_proj(
                        self.deep_promptt_embeddings[idx-1]).expand(B, -1, -1))
