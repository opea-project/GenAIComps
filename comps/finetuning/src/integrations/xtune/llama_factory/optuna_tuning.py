...
class Qwen2VLTuner:
    ...
    def build_training_command(self, trial_params: Dict[str, Any], trial_output_dir: Path) -> list:
        ...
        return [
            ...
            "--stage", "sft",
            "--do_train", "True",
            "--model_name_or_path", f"{cfg.base.model_base_path}/{cfg.base.model_name}",
            "--preprocessing_num_workers", str(cfg.training.preprocessing_num_workers),
            "--finetuning_type", cfg.training.finetuning_type,
            "--template", "qwen2_vl",  # Fixed value
            "--flash_attn", "auto",  # Fixed value
            "--dataset_dir", "data",  # Fixed value
            "--dataset", cfg.training.train_dataset,  # From training config
            "--cutoff_len", str(cfg.training.train_cutoff_len),  # From training config
            "--learning_rate", str(trial_params["learning_rate"]),
            "--num_train_epochs", str(cfg.training.n_epochs),
            "--max_samples", str(cfg.training.max_samples),
            ...
            "--max_grad_norm", str(cfg.training.max_grad_norm),
            "--logging_steps", str(cfg.training.logging_steps),
            "--save_steps", str(cfg.training.save_steps),
            "--warmup_steps", str(trial_params["warmup_steps"]),
            "--packing", "False",   # Fixed value
            "--report_to", "none",  # Fixed value
            "--output_dir", str(trial_output_dir),
            "--bf16", str(cfg.training.bf16),
            "--plot_loss", "True", # Fixed value
            "--ddp_timeout", "180000000",# Fixed value
            "--optim", cfg.training.optim,
            "--video_fps", str(cfg.training.video_fps),
            "--lora_rank", str(cfg.training.lora_rank),
            "--lora_alpha", str(cfg.training.lora_alpha),
            "--lora_dropout", str(cfg.training.lora_dropout),
    ...
    def build_evaluation_command(self, checkpoint_path: str, eval_output_dir: str) -> list:
        ...
        return [
            "llamafactory-cli", "train",
            "--stage", "sft",# Fixed value
            "--do_predict", "True",# Fixed value
            "--model_name_or_path", f"{cfg.base.model_base_path}/{cfg.base.model_name}",
            "--adapter_name_or_path", checkpoint_path,
            "--preprocessing_num_workers", str(cfg.training.preprocessing_num_workers),
            "--finetuning_type", cfg.training.finetuning_type,
            "--quantization_method", cfg.evaluation.quantization_method,
            "--template", "qwen2_vl",  # Fixed value
            "--flash_attn", "auto",  # Fixed value
            "--dataset_dir", "data",  # Fixed value
            "--eval_dataset", cfg.evaluation.eval_dataset,  # From evaluation config
            "--cutoff_len", str(cfg.evaluation.eval_cutoff_len),  # From evaluation config
            "--max_samples", str(cfg.evaluation.max_samples),
            "--per_device_eval_batch_size", str(cfg.evaluation.per_device_eval_batch_size),
            "--predict_with_generate", str(cfg.evaluation.predict_with_generate),
            "--max_new_tokens", str(cfg.evaluation.max_new_tokens),
            "--top_p", str(cfg.evaluation.top_p),
            "--temperature", str(cfg.evaluation.temperature),
            "--video_fps", str(cfg.evaluation.video_fps),
    ...
    def run_training(self, trial_params: Dict[str, Any], trial_number: int) -> str:
        ...
        try:
            # Run training
            logger.info(f"Trial {trial_number}: Executing training command in directory: {self.config.base.working_directory}")
...
def main():
    ...
    logger.info(f"Execution mode detection:")
