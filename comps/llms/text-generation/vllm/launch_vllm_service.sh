...
if [ "$hw_mode" = "hpu" ]; then
    ...
    docker run -d --rm --name="vllm-service" -p $port_number:80 -v $volume:/data -e HTTPS_PROXY=$https_proxy -e HTTP_PROXY=$https_proxy -e HF_TOKEN=${HUGGINGFACEHUB_API_TOKEN} --entrypoint /usr/bin/bash vllm:cpu -c "cd / && export VLLM_CPU_KVCACHE_SPACE=40 && python3 -m vllm.entrypoints.openai.api_server --model $model_name --host 0.0.0.0 --port 80"
fi
