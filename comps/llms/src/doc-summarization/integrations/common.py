    ...
    async def generate(self, input: DocSumChatCompletionRequest, client):
        ...
        if input.summary_type == "stuff":
            ...
            if input.summary_type == "refine":
                ...
                if MAX_TOTAL_TOKENS <= input.max_tokens + 256 or MAX_INPUT_TOKENS < 256:  # 256 is reserved token length for prompt
                    raise RuntimeError("Please set MAX_TOTAL_TOKENS larger than max_tokens + 256, MAX_INPUT_TOKENS larger than 256)")
