...
class PhiOConfig(PretrainedConfig):
    ...
    Args:
        vocab_size (`int`, *optional*, defaults to 200064):
            Vocabulary size of the Phi-O model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`PhiOModel`].
    ...
    ```python
    >>> from transformers import PhiOModel, PhiOConfig

    >>> # Initializing a Phi-O style configuration
    >>> configuration = PhiOConfig.from_pretrained("TBA")

    >>> # Initializing a model from the configuration
    >>> model = PhiOModel(configuration)
