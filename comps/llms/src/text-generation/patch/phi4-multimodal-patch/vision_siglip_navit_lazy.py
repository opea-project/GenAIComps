...
class SiglipTextTransformer(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_mask = expand_2d_attention_mask(attention_mask)
...
sys.path.append("/root/.cache/huggingface/modules/transformers_modules/phi-4-multimodal")
...
###############################################################################
# 1) Imports & checks for Gaudi fused kernels
###############################################################################
...
try:
    ...
    FusedSDPA = None
    _GAUDI_FUSED_SDPA_AVAILABLE = False
...
class ModuleFusedSDPA(torch.nn.Module):
    ...
    def forward(self, query, key, value, attn_mask, dropout_p, is_causal, scale):
        # is_causal can remain False for Siglip since it’s not using causal masks
...
def expand_2d_attention_mask(mask_2d: torch.Tensor) -> torch.Tensor:
    ...
    # mask_2d is shape [batch_size, seq_len], containing 1 for "attend" and 0 for "mask out"
    ...
    # (3) Convert 1.0→0.0 and 0.0→-∞ if the attention code expects additive mask
    extended_mask = (1.0 - extended_mask) * -1.0e4
...
class GaudiSiglipAttention(SiglipAttention):
    ...
    def forward(
        ...
        Args:
            ...
            attention_mask: [batch_size, 1, seq_len, seq_len] if provided.
            output_attentions: If True, returns attention probabilities (not supported by fused kernel).
            use_flash_attention: If True and FusedSDPA is available, uses Habana’s fused kernel.
        """
        # If user wants raw attention outputs, we have to fallback
        if output_attentions and use_flash_attention:
            logger.warning(
                "GaudiSiglipAttention: output_attentions=True is not currently "
                "supported by FusedSDPA. Falling back to the default PyTorch path."
            )
        ...
        if k_len != q_len and k_len != 1:
            use_fused_kernel = False
        else:
            use_fused_kernel = True

        if use_flash_attention and self.fused_scaled_dot_product_attention is not None and use_fused_kernel:
            ...
            attn_weights = None  # the fused kernel doesn't produce attention weights
        else:
            # Original fallback path
            attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) * self.scale

            if attn_weights.size() != (batch_size, self.num_heads, q_len, q_len):
                raise ValueError(
                    f"Attention weights size mismatch. Expected {(batch_size, self.num_heads, q_len, q_len)}, "
                    f"got {attn_weights.size()}"
                )
            if attention_mask is not None:
                if attention_mask.size() != (batch_size, 1, q_len, q_len):
                    raise ValueError(
                        f"Attention mask size mismatch. Expected {(batch_size,1,q_len,q_len)}, "
...
class GaudiSiglipEncoderLayer(SiglipEncoderLayer):
    ...
    def forward(
        ...
        # Pass the new argument down to GaudiSiglipAttention
        hidden_states, attn_weights = self.self_attn(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            output_attentions=output_attentions,
...
class GaudiSiglipEncoder(SiglipEncoder):
    ...
    def __init__(self, config: SiglipConfig):
        ...
        self.layers = nn.ModuleList([GaudiSiglipEncoderLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        # identical logic to SiglipEncoder, just pass down `use_flash_attention` to each layer
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        for layer in self.layers:
            ...
            # If you want to mark steps for HPU graph capturing:
            htcore.mark_step()
...
############### TODO: REMOVE ####################
SiglipAttention = GaudiSiglipAttention
SiglipEncoder = GaudiSiglipEncoder
SiglipEncoderLayer = GaudiSiglipEncoderLayer
############### TODO: REMOVE ####################
