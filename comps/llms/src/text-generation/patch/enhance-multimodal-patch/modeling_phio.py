...
class PhiOImageEmbedding(nn.Module):
    ...
    def forward(self, input_ids: torch.LongTensor, input_embeds: torch.FloatTensor, image_sizes=None, **kwargs) -> torch.FloatTensor:
        ...
        if len(positions.tolist()) > 0:
            ...
            if self.training:
                img_embeds = torch.zeros(1, 3, self.crop_size, self.crop_size, dtype=torch.bfloat16, device=input_ids.device)
...
class PhiOImageAudioEmbedding(nn.Module):
    ...
    def forward(
        ...
        hidden_states = image_hidden_states * image_position_mask.to(torch.bfloat16).unsqueeze(-1) + audio_hidden_states * non_image_position_mask.to(torch.bfloat16).unsqueeze(-1)
