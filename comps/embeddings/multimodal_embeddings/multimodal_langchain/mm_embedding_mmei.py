    ...
    try:
        ...
        if response.status_code != 200:
            return JSONResponse(status_code=503, 
                                content={"message" : "Multimodal embedding endpoint failed!"})
        ...
        res = JSONResponse(status_code=503, 
                            content={"message" : "Multimodal embedding endpoint not started!"})
...
if __name__ == "__main__":
    ...
    mmei_embedding_endpoint = os.getenv("MMEI_EMBEDDING_ENDPOINT", 
                                        f"{url_endpoint}:{port_endpoint}{path_endpoint}")
