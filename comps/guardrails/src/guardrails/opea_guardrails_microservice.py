...
usvc_config = {**dotenv_values(".env"), **os.environ}
...
async def safety_guard(
    ...
    LLMParamsDoc, GeneratedDoc, ChatCompletionRequest, SearchedDoc, ChatCompletionRequest, DocSumChatCompletionRequest
]:
    ...
    try:
        if isinstance(input, LLMParamsDoc):
            ...
            statistics_dict["opea_service@guardrails"].append_latency(
                time.time() - start_time, f"output_guard:{type(input).__name__}"
            )
        ...
        if e.status_code == 466:
            ...
            statistics_dict["opea_service@guardrails"].append_latency(
                time.time() - start_time, f"rejection:{e.status_code}"
            )
        raise e

    except Exception as e:
        logger.error(f"Unexpected error: {str(e)}")
        statistics_dict["opea_service@guardrails"].append_latency(time.time() - start_time, f"error:{type(e).__name__}")
