...
```bash
cd ../../../../
docker build -t opea/dataprep-on-ray-redis:latest --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/dataprep/langchain/redis_ray/Dockerfile .
```
...
```bash
cd comps/dataprep/langchain/redis_ray
docker compose -f docker-compose-dataprep-redis-ray.yaml up -d
```
