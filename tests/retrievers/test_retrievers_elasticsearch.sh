...
function build_docker_images() {
    cd $WORKPATH
    docker build --no-cache -t opea/retriever-elasticsearch:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/retrievers/src/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    ...
    retriever_port=7000
    docker run -d --name="test-comps-retriever-elasticsearch-ms" -p $retriever_port:7000 --ipc=host -e http_proxy=$http_proxy -e https_proxy=$https_proxy -e ES_CONNECTION_STRING=$ES_CONNECTION_STRING -e ES_INDEX_NAME=$INDEX_NAME -e RETRIEVER_COMPONENT_NAME="OPEA_RETRIEVER_ELASTICSEARCH" opea/retriever-elasticsearch:comps
    sleep 15s
