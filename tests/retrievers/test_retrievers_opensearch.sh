...
function build_docker_images() {
    cd $WORKPATH
    docker build -t opea/retriever-opensearch:latest --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/retrievers/src/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    ...
    docker run -d \
        --name test-comps-retriever-opensearch-vectordb \
    ...
    docker run -d \
        ...
        -e OPENSEARCH_INDEX_NAME=$INDEX_NAME \
        -e TEI_EMBEDDING_ENDPOINT=${TEI_EMBEDDING_ENDPOINT} \
        -e HUGGINGFACEHUB_API_TOKEN=$HF_TOKEN \
        -e RETRIEVER_COMPONENT_NAME="OPEA_RETRIEVER_OPENSEARCH" \
        -e LOGFLAG=true \
