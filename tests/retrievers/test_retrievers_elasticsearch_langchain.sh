...
function build_docker_images() {
    cd $WORKPATH
    docker build --no-cache -t opea/retriever-elasticsearch:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/retrievers/src/elasticsearch/langchain/Dockerfile .
    if [ $? -ne 0 ]; then
