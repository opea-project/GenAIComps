...
function build_docker_images() {
    ...
    docker build --no-cache \
        -t opea/llm-ollama-llamaindex:latest \
        --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy \
        -f comps/llms/text-generation/ollama/llama_index/Dockerfile .

    if [ $? -ne 0 ]; then
        echo "opea/llm-ollama-llamaindex built fail"
        exit 1
    else
...
function start_service() {
    docker run -d \
     --name="test-comps-llm-ollama-llamaindex-server" \
     --network host \
...
function stop_docker() {
    cid=$(docker ps -aq --filter "name=test-comps-llm-ollama-llamaindex*")
