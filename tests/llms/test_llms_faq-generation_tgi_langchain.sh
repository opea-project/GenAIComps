...
function build_docker_images() {
    cd $WORKPATH
    docker build --no-cache -t opea/llm-faqgen-tgi:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/llms/faq-generation/tgi/langchain/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    tgi_endpoint_port=5073
    export your_hf_llm_model="Intel/neural-chat-7b-v3-3"
    # Remember to set HF_TOKEN before invoking this test!
    export HF_TOKEN=${HF_TOKEN}
    docker run -d --name="test-comps-llm-faq-tgi-endpoint" -p $tgi_endpoint_port:80 -v ./data:/data -e http_proxy=$http_proxy -e https_proxy=$https_proxy --shm-size 1g ghcr.io/huggingface/text-generation-inference:1.4 --model-id ${your_hf_llm_model}
    export TGI_LLM_ENDPOINT="http://${ip_address}:${tgi_endpoint_port}"

    faq_port=5074
    docker run -d --name="test-comps-llm-faq-tgi-server" -p ${faq_port}:9000 --ipc=host -e http_proxy=$http_proxy -e https_proxy=$https_proxy -e TGI_LLM_ENDPOINT=$TGI_LLM_ENDPOINT -e HUGGINGFACEHUB_API_TOKEN=$HF_TOKEN opea/llm-faqgen-tgi:comps
...
function validate_microservice() {
    ...
    docker logs test-comps-llm-faq-tgi-endpoint

    URL="http://${ip_address}:$faq_port/v1/faqgen"
