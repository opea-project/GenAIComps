...
function build_docker_images() {
    cd $WORKPATH
    docker build --no-cache -t opea/llm-faqgen:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/llms/src/faq-generation/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    export LLM_ENDPOINT_PORT=5073
    export FAQ_PORT=5074
