...
function build_docker_images() {
    ...
    docker build --no-cache -t opea/langchain-mosec:comps --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy -f comps/3rd_parties/mosec/deployment/docker/Dockerfile .
    if [ $? -ne 0 ]; then
