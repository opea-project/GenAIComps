...
function build_docker_images() {
    ...
    docker build --no-cache -t opea/dataprep-pgvector:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f $WORKPATH/comps/dataprep/pgvector/docker/Dockerfile .
    if [ $? -ne 0 ]; then
