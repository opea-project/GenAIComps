...
function build_docker_images() {
    ...
    cd $WORKPATH
    docker build --no-cache -t opea/dataprep-on-ray-redis:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/dataprep/lagnchain/redis_ray/Dockerfile .
    if [ $? -ne 0 ]; then
