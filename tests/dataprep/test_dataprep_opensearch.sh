...
function build_docker_images() {
    cd $WORKPATH
    echo $(pwd)
    docker build -t opea/dataprep-opensearch:latest --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy -f comps/dataprep/src/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    ...
    docker run -d \
        ...
        -e DATAPREP_COMPONENT_NAME="OPEA_DATAPREP_OPENSEARCH" \
