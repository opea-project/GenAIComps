...
function build_docker_images() {
    cd $WORKPATH
    echo $(pwd)
    docker build -t opea/finetuning-rerank:comps --build-arg https_proxy=$https_proxy --build-arg http_proxy=$http_proxy --build-arg HF_TOKEN=$HF_TOKEN -f comps/finetuning/Dockerfile .
    if [ $? -ne 0 ]; then
...
function start_service() {
    ...
    docker run -d --name="test-comps-finetuning-rerank" -p $finetuning_service_port:$finetuning_service_port -p $ray_port:$ray_port --runtime=runc --ipc=host -e http_proxy=$http_proxy -e https_proxy=$https_proxy -e no_proxy=$no_proxy opea/finetuning-rerank:comps
    sleep 1m
}
...
EOF
    ...
    if [ "$HTTP_STATUS" -ne "200" ]; then
        ...
        docker logs test-comps-finetuning-rerank >> ${LOG_PATH}/finetuning-server_upload_file.log
        exit 1
    else
    ...
    if [ "$HTTP_STATUS" -ne "200" ]; then
        echo "[ $SERVICE_NAME ] HTTP status is not 200. Received status was $HTTP_STATUS"
        docker logs test-comps-finetuning-rerank >> ${LOG_PATH}/finetuning-server_create.log
        exit 1
    else
        echo "[ $SERVICE_NAME ] HTTP status is 200. Checking content..."
    fi
...
function stop_docker() {
    cid=$(docker ps -aq --filter "name=test-comps-finetuning-rerank*")
