...
class TestMicroService(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.process = multiprocessing.Process(target=opea_microservices["mcp_dummy"].start, daemon=False, name="mcp_dummy")
        self.process.start()
    ...
    async def test_mcp(self):
        ...
        async with self.mcp_client:
            ...
            result = await self.mcp_client.call_tool("mcp_dummy", {"request": {"text": "Hello "}},)
            self.assertEqual(json.loads(result[0].text)["text"], "Hello OPEA Project MCP!")

            result = await self.mcp_client.call_tool("mcp_dummy_sum",)
